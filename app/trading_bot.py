...
async def main():
    ...
    await bot.polling(non_stop=True)  # ✅ Работает без блокировки event loop

if __name__ == "__main__":
    nest_asyncio.apply()  # ✅ Разрешает многократный запуск event loop
    loop = asyncio.new_event_loop()  # ✅ Создаём чистый event loop
    asyncio.set_event_loop(loop)
    loop.run_until_complete(main())  # ✅ Запускаем бот без зависаний